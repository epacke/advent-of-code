...
export class PipeNetwork {
  ...
  cleanLoop(){
    this.map = this.map
      .map((row, y) => row.map((col, x) => !this.isInPath({y, x}) ? '.': col));
  }
}
...
const polygon: Point[] = network.path.map(c => [c.y, c.x])
const c = network.map
  .map((row, y) => {
    return row.map((c, x) => classifyPoint(polygon, [y, x])).filter(c => c === -1).length;
  }).reduce((a,b) => a + b);
