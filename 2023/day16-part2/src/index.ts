...
function BeamMeUp(grid: string[][], start: Vector[]) {
      ...
      while (true) {
        ...
        if (direction === Direction.RIGHT || direction === Direction.LEFT) {
          ...
          if (char === '|') {
            ...
            nextSteps.forEach(s => {
              pathsToTry.set(JSON.stringify(s), s)
            })
            break
          }
        }
...
const startPositions: Vector[][] = [];

for (let y = 0; y < grid.length; y++){
  startPositions.push([[-1, y, Direction.RIGHT]])
  startPositions.push([[grid[0].length, y, Direction.LEFT]])
}

for (let x = 0; x < grid[0].length; x++){
  startPositions.push([[x, -1, Direction.DOWN]])
  startPositions.push([[x, grid.length, Direction.UP]])
}

console.log(Math.max(...startPositions.map(s => BeamMeUp(grid, s))));
