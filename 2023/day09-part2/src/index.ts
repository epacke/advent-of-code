...
class OASIS {
  ...
  calculatePreviousValue = (extrapolations: number[][]) => {
    extrapolations[extrapolations.length-1] = [0, ...extrapolations[extrapolations.length-1]]
    let calculation = 0;
    for (let i = extrapolations.length-1; i > 0; i--){
      const extrapolation = extrapolations[i];
      const curFirstDigit = extrapolation[0];
      const previousRow = extrapolations[i - 1];
      const prevFirstDigit = previousRow[0];
      calculation = prevFirstDigit - curFirstDigit;
      extrapolations[i - 1] = [calculation, ...extrapolations[i - 1]];
    }
    return calculation;
  }
}
...
const sum = input.split(/\n/).map(reading => {
  ...
  o.extrapolate();
  return o.calculatePreviousValue(o.extrapolations);
}).reduce((a, b) => a+b)
console.log(sum)
